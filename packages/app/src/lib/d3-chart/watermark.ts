...
/** Create a diagonal repeating "EXPERIMENTAL - DAY ZERO" watermark pattern (blue). */
export function createDay0Watermark(
  svg: d3.Selection<SVGSVGElement, unknown, null, undefined>,
  defs: d3.Selection<SVGDefsElement, unknown, null, undefined>,
  innerWidth: number,
  innerHeight: number,
  ...
  const patternId = `day0-pattern-${chartId}`;
  // Brick pattern: two rows per tile, second row shifted by half-width.
  // Width controls spacing along the text axis; rowHeight controls perpendicular row spacing.
